  ...
  try {
    ...
    if (!orderAnalytics) {
      ...
      res.status(200).send({
        success: true,
        message: "",
        totalPrice: orderAnalytics.pop().totalsales,
...
const orderPending = async (req, res) => {
  try {
    const orderDetails = await Order.find({
      ...
      .populate("user", "name country_code token")
      .populate({
        path: "orderItems",
        populate: [{ path: "product" }],
      })
...
const orderDelivered = async (req, res) => {
  try {
    const orderDetails = await Order.find({
      ...
      .populate("user", "name country_code")
      .populate({
        path: "orderItems",
        populate: { path: "product" },
      })
...
const getOrderByUserId = async (req, res) => {
  try {
    await Order.find({ user: req.params.id, status: { $ne: "PENDING" } })
      .sort({ dateOrdered: -1 })
      .populate({
        path: "orderItems",
        populate: { path: "product" },
...
const editOrderStatus = async (req, res) => {
  try {
    if (!mongoose.isValidObjectId(req.params.id)) {
      return res
        .status(500)
        .send({ error: "Invalid order id", success: false });
    }
...
const newOrder = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .send({ success: false, message: "Bad request", error: error.message });
  }
};
