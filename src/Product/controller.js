...
const getProductList = async (req, res) => {
  ...
  const ProductList = await Products.find({ status: "ACTIVE" })
    .populate("category")
    .populate("size_list", "_id")
    .skip((page - 1) * limit)
    .limit(limit)
    .exec();
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching product list", error });
  }
};
...
const getRejectedStatusProductList = async (req, res) => {
  ...
  const ProductList = await Products.find({ status: "REJECTED" }).select(
    selectedFields
  );
  if (!ProductList) {
    res.status(500).send({ success: false });
...
const getTopPicksData = async (req, res) => {
  ...
  await Products.aggregate([{ $sample: { size: pageSize } }, { $skip: skip }], {
    status: "ACTIVE",
  })
    .then((response) => {
      return res
...
const searchProduct = async (req, res) => {
  try {
    const SearchResult = await Products.find(
      { $or: [{ name: new RegExp(req.params.id, "i") }] },
      { status: "ACTIVE" }
    );
    if (!SearchResult) {
...
const getProductListBySeller = async (req, res) => {
  ...
  const totalPages = Math.ceil(totalCount / limit);
  await Products.find({ productOwner: req.params.id })
    .populate("productOwner", "shopName")
    .populate(
      "category",
      "name showDimensions showSizeSelection parentCategory"
    )
    .skip((page - 1) * limit)
    .limit(limit)
    .exec()
...
const getProductById = async (req, res) => {
  await Products.find({ _id: req.params.id })
    ...
    .then((response) => {
      return res
        .status(201)
        .send({ success: true, message: "", data: response[0] });
    })
    .catch((error) => {
      return res.status(500).send({ success: false, error: error.message });
    });
...
const getProductAnalytics = async (req, res) => {
  ...
  if (!ProductCount) {
    ...
    res
      .status(201)
      .send({ success: true, message: "", total_product: ProductCount });
  }
};
...
const editProduct = async (req, res) => {
  if (!mongoose.isValidObjectId(req.params.id)) {
    return res
      .status(500)
      .send({ error: "Invalid product id", success: false });
  }
